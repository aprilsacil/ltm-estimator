jQuery(function($) {
    ...
    (function() {
         ...
         * Suggestion Field
         */
        $(window).on('compute-cost-click', function(e, target) {
            e.preventDefault();
            $(target).parents('form').find('.help-text').text('');
            $(target).parents('form').find('.form-group').removeClass('has-error');
            $('#cost-estimate').addClass('d-none');

            // get data
            let formdata = $(target).parents('form').serializeArray();
            data = {};
            ...
            let errors = false;
            // validate cases
            if (typeof data.cases == 'undefined' || data.cases == '' || parseFloat(data.cases) == 'NaN') {
                errors = true;
                $('#cases').addClass('has-error');
                $('#cases .help-text').text('Please provide a valid number of cases');
            }
            
            // validate growth
            if (typeof data.growth == 'undefined' || data.growth == '' || !(/^[0-9]+(\.)?[0-9]*$/.test(data.growth))) {
                errors = true;
                $('#growth').addClass('has-error');
                $('#growth .help-text').text('Please provide a valid number of growth percentage');
            }
            // validate months
            if (typeof data.months == 'undefined' || data.months == '') {
                errors = true;
                $('#months').addClass('has-error');
                $('#months .help-text').text('Please provide a valid number of months');
            }

            if (errors) { return; }

            // if no errors throw ajax call
            $.post("/cost/calculator", data)
            .done(function(response) {
                ...
                if (response.error) {
                    ...
                    for (var i in response.validation) {
                        ...
                        $('#' + i + ' .help-text').text(response.validation[i]);
                    }
                } else {
                    $('#cost-estimate #total-estimate').html('Total cost estimate for ' + data.months + ' months: <strong>' + response.results.total_cost + ' USD</strong>');
                    $('#cost-estimate table tbody').html('');
                    $('#cost-estimate table tfoot').html('');
                    for (var i in response.results.breakdown) {
                        $('#cost-estimate table tbody').append('<tr />')
                            .append('<td>' + response.results.breakdown[i].month + '</td>')
                            .append('<td>' + response.results.breakdown[i].cases + '</td>')
                            // .append('<td>' + response.results.breakdown[i].ram_consumed_mb + '</td>')
                            // .append('<td>' + response.results.breakdown[i].storage_consumed_mb + '</td>')
                            .append('<td>' + response.results.breakdown[i].cost + '</td>')
                    }
                    $('#cost-estimate table tfoot').append('<tr />')
                        .append('<td>TOTAL</td>')
                        .append('<td>' + response.results.total_cases + '</td>')
                        // .append('<td>' + response.results.total_ram_consumed_mb + '</td>')
                        // .append('<td>' + response.results.total_storage_consumed_mb + '</td>')
                        .append('<td>' + response.results.total_cost + '</td>');
                        $('#cost-estimate').removeClass('d-none');
                }
            })
        });
    })();
